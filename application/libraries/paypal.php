...
class paypal {
   ...
   function paypal() {
      ...
      $this->paypal_url = ($this->production == TRUE) ? $this->production_url : $this->sandbox_url;
      $this->last_error = '';
      
      $this->ipn_log_file = 'assets/paypal.log';
      $this->ipn_log = true; 
      $this->ipn_response = '';
         ...
         else
         {
           ...
           while (!feof($fp)) {
             ...
             if (stripos($res, "VERIFIED") !== false) {
              ...
              /* $payment_id = trim($_POST['custom']);
               $payment_details = array('payment_status'=>'completed');
               $this->payment_mdl->save($payment_details, $payment_id);
               $payment_data = $this->payment_mdl->get($payment_id, true);
               $invoice_id = $payment_data->invoice_id;
               $this->invoice_mdl->set_invoice_status($invoice_id);
               $this->notifications->payment_received($invoice_id);*/
               return true;
             }

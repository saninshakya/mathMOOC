...
class Auth extends MY_Controller {
	...
	{  
		...
		if ($this->form_validation->run() == false)
		{
			...
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$this->template->title('Online Exam System | Forgot Password ')
	        ->set_layout($this->auth_tpl)
	        ->set('page_title', 'Forgot Password')
	        ->build($this->auth_folder.'/forgot_password', $this->data);
		}
		else
		{
			...
			$forgotten = $this->ion_auth->forgotten_password($identity->{$this->config->item('identity', 'ion_auth')}, $settings);
